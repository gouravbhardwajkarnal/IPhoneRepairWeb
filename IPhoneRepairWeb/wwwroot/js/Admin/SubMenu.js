angular.module("myApp")
    .controller("SubMenuCtrl", ['commonService', '$scope', '$timeout', '$window', '$filter', 'MasterMenu', '$http', function (commonService, $scope, $timeout, $window, $filter, MasterMenu, $http) {
        ...
        $scope.SaveClick = function () {
            ...
            commonService.post("https://localhost:5001/api/Admin/CreateSubMenu", Data).then(function (response) {
                data = response.data;
                if (data == 0) {
        ...
        $scope.GetSubMenuAll = function () {
            commonService.get("https://localhost:5001/api/Admin/GetALLSubMenu", {}).then(function (response) {
                data = response.data;
                if (data != '') {
        ...
        $scope.UpdateClick = function () {
            ...
            Data = JSON.stringify(dataobject);
            commonService.post("https://localhost:5001/api/Admin/UpdateSubMenu", Data).then(function (response) {
                data = response.data;
                if (data == 0) {
                    $scope.MenuName =[0]["--Select--"];
                    $scope.SubMenu = "";
        ...
        $scope.DeleteMenu = function (index, Autoid) {
            if (confirm("Are you sure want to delete ?")) {
                commonService.post("https://localhost:5001/api/Admin/DeleteSubMenu?Id=" + Autoid + "", {}).then(function (response) {
                    data = response.data;
                    if (data == 'Deleted') {
                        $scope.MenuName = [0]["--Select--"];
                        $scope.SubMenu = "";
